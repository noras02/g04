...
```js
match (p: Person)
where p.born = 1964 or p.born = 1965 /* where p.born in [1964,1965]*/
...
```js
MATCH(p: person)-[:produced]->(m:movie) /*where nem kell: MATCH(p: person {name: 'Joel Silver'})-[:produced]->(m:movie) */
...
```js
MATCH(p:Person)-[:DIRECTED]->(m:Movie) /*nincs GROUP BY, HAVING*/
WITH p.name AS Nev, COUNT(m.title) AS Darab/*segéd lekérdezés*/
WHERE Darab >1 /*HAVING*/
RETURN Nev, Darab
```
...
*/
/*
WHERE EXISTS MATCH (p:Person)-[:DIRECTED]->(m:Movie)
*/
RETURN p.name
```
